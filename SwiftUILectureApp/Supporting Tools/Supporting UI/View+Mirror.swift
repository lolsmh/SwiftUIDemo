...
struct MirrorHierarchyView: ViewModifier {
...
struct Diagram<V: View>: View {
	let tree: Tree<Unique<Colored>>
	var strokeWidth: CGFloat = 1
	let node: (Binding<NodeLayoutState>, Binding<Bool>, Unique<Colored>) -> V
	@State private var layoutState: NodeLayoutState
	@State private var collapsed: Bool
	@Environment(\.colorScheme) private var colorScheme
	
	let collapsedDefault: Bool
	init(
		tree: Tree<Unique<Colored>>,
		strokeWidth: CGFloat = 1,
		node: @escaping (Binding<NodeLayoutState>, Binding<Bool>, Unique<Colored>) -> V,
		...
		self._layoutState = State(initialValue: .empty)
		self._collapsed = State(initialValue: !tree.children.isEmpty && collapsed)
	}
	
	private typealias Key = CollectDict<Unique<Colored>.ID, Anchor<CGPoint>>
			...
			.anchorPreference(
				...
				.onTapGesture {
					withAnimation {
						switch layoutState {
						case .empty:
							layoutState = .offeredSpace
						case .offeredSpace:
							layoutState = .providedSpace
							collapsed = true
						case .providedSpace:
							layoutState = .empty
							collapsed.toggle()
				...
				HStack(
					...
					ForEach(
						...
						Diagram(
							...
							node: self.node,
							collapsed: self.collapsedDefault
						)
					}
				}
			}
		}
		.backgroundPreferenceValue(Key.self) { (centers: [Unique<Colored>.ID: Anchor<CGPoint>]) in
		...
		.background(
			RoundedRectangle(cornerRadius: 12)
				.fill(
					LinearGradient(
						gradient: Gradient(
							colors: Color.keynoteLikeGradient(
								hue: Double(hue.value)/360
							)
						),
						startPoint: .top,
						endPoint: .bottom
...
struct MirrorView<Content>: View {
	...
	var body: some View {
		let tree = Tree(reflecting: content)
			.simplified(rules: rules)
			.map(Unique.init)
		
		return Diagram(tree: tree) { $layoutState, $collapsed, value in
			Text(value.value.text)
				.bold()
				.fixedSize()
				.modifier(MyNode(hue: value.value.hue ?? layoutState.hue))
		}
...
extension Tree where A == String {
	...
	private func _simplified(rules: [Rule], _ _state: DiagramState) -> Tree<Colored> {
		...
		if value == "Optional" && children.count == 1 {
			...
			return Tree<Colored>(
				Colored(child.value + "?", hue: state.hue),
				children: child.children.map { $0._simplified(rules: rules, childState) }
			)
		...
		return Tree<Colored>(
			Colored(value.simplerName, hue: state.hue),
			children: children.map { $0._simplified(rules: rules, childState) }
		)
	}
}
...
struct Hue: Hashable {
	...
	static let black = Hue(value: 180)
	static let lol = Hue(value: 45)
}
...
extension Rule {
	static let highlightSubtreeOfEnvironment = Rule(
		apply: { tree, state, childState in
	...
	static let highlightPreferenceAncestors = Rule(
		apply: { tree, state, childState in
